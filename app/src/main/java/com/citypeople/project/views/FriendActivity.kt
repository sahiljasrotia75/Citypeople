...
class FriendActivity : BaseActivity(), FriendListener, FriendAdapter.FriendItemListener {
    ...
    private val mStateCallback: CameraDevice.StateCallback? =
        object : CameraDevice.StateCallback() {
            ...
            override fun onError(cameraDevice: CameraDevice, error: Int) {
                ...
                cameraDevice.close()
                mCameraDevice = null
                val activity: Activity = this@FriendActivity
                if (null != activity) {
                    activity.finish()
                }
            }
    ...
    private fun chooseOptimalSize(
        ...
        return if (bigEnough.size > 0) {
            Collections.min(bigEnough, FriendActivity.CompareSizesByArea())
        } else {
        ...
        try {
            for (cameraId in manager.cameraIdList) {
                ...
                val largest = Collections.max(
                    ...
                    FriendActivity.CompareSizesByArea()
                )
