...
class VideoSendActivity : BaseActivity(), VideoSendListener,
     ...
     * Tag for the [Log].
     */
    private val TAG = "VideoSendActivity"
    ...
    private val mStateCallback: CameraDevice.StateCallback? =
        object : CameraDevice.StateCallback() {
            ...
            override fun onError(cameraDevice: CameraDevice, error: Int) {
                ...
                cameraDevice.close()
                mCameraDevice = null
                val activity: Activity = this@VideoSendActivity
                if (null != activity) {
                    activity.finish()
                }
            }
    ...
    private fun chooseOptimalSize(
        ...
        return if (bigEnough.size > 0) {
            Collections.min(bigEnough, VideoSendActivity.CompareSizesByArea())
        } else {
        ...
        try {
            for (cameraId in manager.cameraIdList) {
                ...
                val largest = Collections.max(
                    ...
                    VideoSendActivity.CompareSizesByArea()
                )
